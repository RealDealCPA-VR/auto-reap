...
def test_archive_moves_losers_but_never_the_winner(
    spec, winner_manifest, report_path, ws, loser_gguf
):
    ...
    assert Path(winner_manifest.path).exists()  # winner source untouched


def test_archive_disabled_leaves_losers(spec, winner_manifest, report_path, ws, loser_gguf):

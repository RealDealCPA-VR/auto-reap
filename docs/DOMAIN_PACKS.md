...
unknown fields and type errors fail loudly with the exact path).
...
| `domains` | list[DomainSpec] | required, ≥ 1 | The workload mix. |
...
| Field | Type | Default | Meaning |
|---|---|---|---|
| `name` | str | required | Domain id. Used in dataset records, per-domain scores, and reports. Keep it a short slug (`tax_research`, `ticket_triage`). Do not use the reserved names `benign_sensitive` or `should_refuse` — those belong to the auto-added suites. |
| `description` | str | required | What this slice of work is. The generator reads it. |
| `task_type` | enum | `open_ended` | How items in this domain are scored — see the scorer table below. |
| `weight` | float > 0 | `1.0` | Share of the runtime mix. Relative, not percent — weights are normalized across the pack. |
...
**Audit before you prune.** `audit` samples generated items for human review — you are
checking that prompts look like your real traffic, gold labels are actually correct, and
rubrics would convince you. Ten minutes here beats a $10 prune run calibrated on junk.

Iterating: dataset generation is keyed into the sweep's config hash, so editing the pack (or
`data:` sizes) and re-running produces a fresh run directory with fresh datasets — your old
runs stay intact for comparison ([ARCHITECTURE.md](ARCHITECTURE.md#reproducibility-config-hash--resume)).
...
|---|---|---|
| `calibration_size` | 1000 | Total calibration prompts (PRD suggests 1,000–2,000). Prompts only — no gold — so they're cheap. |
| `eval_size` | 300 | Total held-out eval items (300–500 recommended). |
| `near_dup_threshold` | 0.90 | Similarity at/above this between a calibration and an eval item blocks the eval item (leakage guard, PRD FR-1.3). |
| `dedup_backend` | `fuzzy` | `fuzzy` = rapidfuzz string similarity, zero extra deps. `embedding` = cosine over an OpenAI-compatible `/v1/embeddings` endpoint (e.g. a local embedding model in LM Studio) — set `embedding_provider:`. |
| `long_context_share` | 0.05 | Fraction of items generated long-context, drawn from domains with `long_context: true`. |
...
frontier provider, and emits a draft pack YAML — domains with names, descriptions, task
types, guessed weights, prompt guidance, and skeleton schemas/tool defs where it inferred
structured work — plus a sweep spec pointing at it.
...
2. **Task types** — anything with one canonical answer should be `exact`/`json_schema`, not
   `open_ended` (cheaper and more objective to score).
